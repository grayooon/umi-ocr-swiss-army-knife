...
2. 解压本插件 ZIP，将 `DDDDOCR_Captcha_Umi` 文件夹复制到 `UmiOCR-data\plugins\`。
3. 核对 `UmiOCR-data\plugins\DDDDOCR_Captcha_Umi\__init__.py` 存在；解释器在 `UmiOCR-data\runtime\python.exe`。不要多套一层解压目录。
...
本版固定 ddddocr 1.5.6，使用其官方包内的 `site-packages/ddddocr/common.onnx`（Beta）与 `common_old.onnx`（经典）。它们随依赖下载，不需要再到 models 文件夹寻找。

起始设置：Beta、小写字母+数字、长度 4、严格长度校验开、定长 CTC 束搜索。如果你的图片不是这个字符范围/长度，请调整；不确定时先关闭严格长度限制。支持经典、双模型和比赛模式；比赛模式更多次推理，不保证所有样本更准。

批量评测：准备单独图片目录，把真实四位答案写在文件名开头，如 `s9px_001.png`、`utw6-hard.png`；运行 `tools/benchmark_folder.bat`，输入该目录路径。输出 `ddddocr_benchmark.tsv` 和各模式整图完全一致率。不能把没有标签的图片数量当作有效评测样本。

自定义模型不是默认部署所需。已有兼容 ONNX 和 charsets.json 时，放进插件 `models/custom/`，在全局设置选择这两个文件，局部模式选择 custom。换电脑时使用插件内相对路径或重新选择文件。
