...
2. 解压本插件 ZIP，将 `HyperLPR3_Plate_CPU` 文件夹复制到 `UmiOCR-data\plugins\`。
3. 核对 `UmiOCR-data\plugins\HyperLPR3_Plate_CPU\__init__.py` 存在；解释器在 `UmiOCR-data\runtime\python.exe`。不要多套一层解压目录。
...
安装器会把 4 个权重下载到插件 `models` 目录：`y5fu_320x_sim.onnx`、`y5fu_640x_sim.onnx`、`rpv3_mdict_160_r3.onnx`、`litemodel_cls_96x_r1.onnx`。模型目录设置留空即可自动查找。

Umi 中选择 HyperLPR3 后，先保持 low 与其他默认设置，用清晰车牌图片测试。high 使用另一个检测模型；是否更适合远处/小车牌需要用自己的图片比较。普通文字图片返回无车牌是合理现象，不能用它验证车牌识别准确性。
